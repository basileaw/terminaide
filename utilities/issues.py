...
def get_repo_info():
    ...
    try:
        ...
        console.print("[red]Error: Not a git repository or no remote origin found[/red]")
        sys.exit(1)
        ...
        if hasattr(e, 'response') and e.response is not None:
            try:
                ...
                if 'message' in error_data:
                    console.print(f"[red]GitHub API error: {error_data['message']}[/red]")
            except:
                pass
        sys.exit(1)
...
def create_issue(repo, title, label, token):
    ...
    console.print(f"[green]✓[/green] Created [{color_code}]{issue_type}[/{color_code}] [green]#{issue['number']}[/green]: [bold]\"{title}\"[/bold]")
    ...
    for issue_num in issue_numbers:
        try:
            ...
            if operation == 'resolve':
                ...
                console.print(f"[green]✓[/green] Resolved issue [green]#{issue_num}[/green]: [bold]\"{issue['title']}\"[/bold]")
                console.print(f"[dim]→ {issue['html_url']}[/dim]")
                ...
                data = {'state': 'closed'}
                make_github_request('PATCH', issue_url, data=data, token=token)
                console.print(f"[red]✔[/red] Closed issue [green]#{issue_num}[/green]: [bold]\"{issue['title']}\"[/bold]")
                console.print("[dim]Note: GitHub API doesn't support deleting issues, so it was closed instead[/dim]")
                
        except Exception as e:
            console.print(f"[red]✗[/red] Issue [green]#{issue_num}[/green] not found or error occurred")
...
def main():
    ...
    create_parser.add_argument('label', choices=['bug', 'task', 'idea'], help='Issue label/type')
    create_parser.add_argument('title', nargs='*', help='Issue title (multiple words allowed)')
    ...
    resolve_parser.add_argument('issues', nargs='*', type=int, help='Issue numbers to resolve')
    
    # Delete command (actually closes)
    delete_parser = subparsers.add_parser('delete', help='Delete (close) issues')
    delete_parser.add_argument('issues', nargs='*', type=int, help='Issue numbers to delete')
    ...
    if args.command == 'create':
        ...
        if not title:
            console.print(f"[red]Error:[/red] Please provide a title: make {args.label} \"Your issue title\"")
            sys.exit(1)
        ...
        if not args.issues:
            console.print("[red]Error:[/red] Please provide issue numbers: make resolve 1 2 3")
            sys.exit(1)
        batch_operation(repo, args.issues, 'resolve', token)
    elif args.command == 'delete':
        if not args.issues:
            console.print("[red]Error:[/red] Please provide issue numbers: make delete 1 2 3")
            sys.exit(1)
