...
def run_game(stdscr, max_y, max_x, high_score=0):
    ...
    last_move=time.time()
    while True:
        if _exit_requested:return score
        now=time.time()
        delta=now-last_move
        last_move=now
        fall_time+=delta*1000
        key=game_win.getch()
        if key in (ord('q'),27):return score
        if key in[curses.KEY_LEFT,ord('a'),ord('A')]:
            np=[pos[0],pos[1]-1]
            if is_valid_position(board,TETROMINOS[piece_type][rotation],np):pos=np
            ...
            if is_valid_position(board,TETROMINOS[piece_type][r],pos):rotation=r
        elif key==ord(' '):
...
def is_valid_position(board,piece,pos):
    bh=len(board);bw=len(board[0])
    for y,x in piece:
        ny=pos[0]+y;nx=pos[1]+x
        if nx<0 or nx>=bw or ny>=bh:return False
        if ny>=0 and board[ny][nx]!=0:return False
    return True

def place_tetromino(board,piece,pos,t):
    for y,x in piece:
        ny=pos[0]+y;nx=pos[1]+x
        if ny>=0:board[ny][nx]=t
    ...
    while y>=0:
        if all(board[y][x]!=0 for x in range(bw)):
            for y2 in range(y,0,-1):
                for x in range(bw):board[y2][x]=board[y2-1][x]
            for x in range(bw):board[0][x]=0
            c+=1
        else:y-=1
    return c
...
def draw_game(stdscr,gw,nw,board,piece,pos,ptype,ntype,score,level,lines,high_score,bh,bw,mx,sy,sx):
    ...
    t="PLAY TETRIS"
    ww=gw.getmaxyx()[1]
    if ww>len(t)+4:gw.addstr(0,(ww-len(t))//2,t,curses.A_BOLD|curses.color_pair(9))
    nw.addstr(0,1,"NEXT",curses.A_BOLD|curses.color_pair(9))
    for y in range(bh):
        for x in range(bw):
            c=board[y][x]
            if c!=0:
                try:gw.addstr(y+1,x*2+1,"[]",curses.color_pair(c)|curses.A_BOLD)
                except:curses.error
    for y,x in piece:
        ny=pos[0]+y;nx=pos[1]+x
        if ny>=0:
            try:gw.addstr(ny+1,nx*2+1,"[]",curses.color_pair(ptype+1)|curses.A_BOLD)
            except:curses.error
    nt=TETROMINOS[ntype][0]
    minx=min(a for _,a in nt);maxx=max(a for _,a in nt)
    miny=min(a for a,_ in nt);maxy=max(a for a,_ in nt)
    cy=3;cx=5
    for yy,xx in nt:
        dy=cy+yy-(miny+maxy)//2
        dx=cx+(xx-(minx+maxx)//2)*2
        try:nw.addstr(dy,dx,"[]",curses.color_pair(ntype+1)|curses.A_BOLD)
        except:curses.error
